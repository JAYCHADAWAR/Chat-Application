...
var _require = require("mysql/lib/protocol/constants/client"),
    CLIENT_SECURE_CONNECTION = _require.CLIENT_SECURE_CONNECTION;

var io = require('socket.io')(server); // register new function
...
var temppath = path.join(__dirname, "../templates/views");
...
app.set("views", temppath); //ejs.registerPartials(partialpath);
...
io.on('connection', function (socket) {
  ...
  socket.on("user_connected", function (id) {
    // save in array
    users[id] = socket.id; // socket ID will be used to send message to individual person
  ...
  socket.on("send_message", function (data) {
    ...
    io.to(socketId).emit("new_message", data); // save in database
...
app.post("/register", function (req, res) {
  ...
  connection.query(sql, function (err) {
    ...
    res.render('index', {
      msg: flash('data saved')
    });
  });
});
app.post("/contactus", function (req, res) {
  var cname = req.body.ctname;
  var cemail = req.body.ctemail;
  var cm = req.body.cttxt;
...
app.post("/get_message", function (req, res) {
  ...
  connection.query("select * from message where outid=? AND incomingid=? OR outid=? AND incomingid=? ORDER BY msgid", [session.userid, req.body.receiver, req.body.receiver, session.userid], function (err, selre, fields) {
    ...
    console.log(selre);
    console.log(req.body.receiver); //session.chat=selre;

    res.send(JSON.stringify(selre));
  });
});
app.post("/get_profilepic", function (req, res) {
...
app.post("/chatpage", function (req, res) {
  ...
  connection.query('SELECT * FROM users WHERE id= ?', [session.userid], function (err, rows, fields) {
    ...
    res.render('chatpage', {
      id: session.userid,
      idd: rows[0].id,
      emaild: rows[0].emailid,
      name: rows[0].name,
      status: rows[0].status,
      dpp: rows[0].dp,
      data: data,
      chats: session.chat,
      secusname: session.namesec,
      secuseid: session.secid,
      pdp: session.profdp
    });
  });
});
